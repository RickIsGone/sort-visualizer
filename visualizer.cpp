...
void Visualizer::run(){
    ...
    if(sdl::button("shuffle",400,100,200,100,60,60,60,m_renderer,m_font)) if(!m_sorter.isSorting()) m_sorter.shuffle(m_array,m_array.size());
    ...
    for(int i:m_array){
        SDL_Rect num;
        num.w=700.f/m_array.size();
        num.h=i*650.f/m_array.size();
        num.y=750.f-num.h;
        num.x=700.f+offsett;
        offsett+=num.w;
